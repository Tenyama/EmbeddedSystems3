// ----------------------------------- cli.c -----------------------------------
// // This file implements the Command Line Interface (CLI) for the OS,
// including features like command history, backspace, delete handling, and
// tab-completion.
...
// Define constants for the maximum size of commands, history size, backspace,
// and delete keys
    ...
    // Handle tab-completion for both regular commands and "help <command>"
      ...
      // Handle regular command completion (without "help ")
        ...
        if (match_count == 1) {
          ...
          uart_puts(cli_buffer);
          index = string_length(
              cli_buffer); // Move the cursor to the end of the command
        }
        // If multiple matches, display all possible completions
        else if (match_count > 1) {
          uart_puts("\nPossible matches:\n");
          ...
          uart_puts("\n*.* Group1_OS *.* > ");
          uart_puts(cli_buffer); // Reprint the command buffer after displaying
                                 // options
        }
      }
    }
    // Handle Enter key (execution of the current command)
      ...
      for (int i = 0; i <= index; i++) {
        history[history_index][i] =
            cli_buffer[i]; // Copy the command to history
      }

      uart_puts("\n"); // New line after the command is entered
      history_index =
          (history_index + 1) % HISTORY_SIZE; // Update history index
