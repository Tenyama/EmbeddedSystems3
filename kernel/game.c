...
{
    return cosine_table[angle / 10]; // Divide by 10 to get the index for the table
}
    ...
    *end_x = base_x + (cosine_val * SHOOTER_LENGTH) / 1000;
    ...
    if (*end_x > 700) {
        ...
        *end_x = 228;
        ...
        float cosine_float = cosine_val / 1000.0;  // Convert to floating point by dividing by 1000
        if (cosine_float < 0) {
            cosine_float = -cosine_float;  // Make sure cosine value is positive
        }

        // Adjust shooter length based on cosine value
        shooter_length_dummy = (int)(241 / cosine_float);  // Use floating point division, then cast back to int
    }
...
// Function to erase the shooter by restoring the part of the background it covered
void eraseShooter(int base_x, int base_y, int shooter_angle)
{
    ...
    for (int i = 0; i < SHOOTER_LENGTH; i++)
    {
        ...
        for (int j = -4; j <= 4; j++)
        {
            // Calculate the x and y positions for the shooter based on angle and thickness
            int x = base_x + (cosine_val * i - sine_val * j) / 1000; // X position considering thickness
            int y = base_y - (sine_val * i + cosine_val * j) / 1000; // Y position considering thickness
