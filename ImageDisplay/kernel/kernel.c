...
{
	mBuf[0] = 8 * 4;		// Message Buffer Size in bytes (17 elements * 4 bytes (32 bit) each)
...
void drawImage(int start_x, int start_y, const unsigned int data[], int width, int height)
{
	...
	for (int y = start_y; y < start_y + height; y++)
	{
		...
		{
			if (myImage[index] != 0)
			{ // only draw if the pixel is not black
...
void main()
{
	...
	// draw_string(222, 540, "Thao Trinh", 0xFFFF00, 4);  // Draw text at position (10,10) with a yellow color
